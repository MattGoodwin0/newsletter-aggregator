...
export default function FeedValidator() {
  ...
  const validate = async (targetUrl) => {
    ...
    try {
      const res = await post("/api/validate", { url: val });
            ...
            <div className="flex gap-2">
              <div className="flex-1 flex items-center gap-2 bg-[#f5f5f7] border border-black/[0.12] rounded-[12px] px-3 py-[10px]">
                ...
                {url && (
                  <button
                    onClick={reset}
                    className="text-[#c7c7cc] hover:text-[#86868b] transition-colors flex-shrink-0"
                  >
                    <X size={12} />
                  </button>
                )}
            ...
            <div className="px-5 py-6 flex flex-col gap-[10px]">
              {[100, 120, 90, 110].map((w, i) => (
                <div key={i} className="flex items-center gap-3">
                  <div
                    className="w-4 h-4 rounded-full bg-black/[0.06] flex-shrink-0"
                    style={{
                      animation: `pulse 1.4s ease-in-out ${i * 0.15}s infinite`,
                    }}
                  />
                  <div
                    className="h-[13px] rounded-full bg-black/[0.05] flex-shrink-0"
                    style={{
                      width: w,
                      animation: `pulse 1.4s ease-in-out ${i * 0.15}s infinite`,
                    }}
            ...
            <div>
              ...
              <div className="px-5 pt-5 pb-2 flex flex-col gap-[2px]">
                {Object.entries(report.checks).map(([id, check]) => (
                  <div
                    key={id}
                    className="flex items-center justify-between py-[9px] border-b border-black/[0.05] last:border-0"
                  >
                    ...
                    <span className="text-[11px] text-[#86868b] text-right max-w-[200px] truncate">
                      {check.detail}
                    </span>
                  </div>
                ))}
                ...
                <div className="mx-5 mb-5 rounded-[12px] border border-black/[0.07] overflow-hidden">
                  {report.sample_article.image && (
                    <div className="h-24 overflow-hidden">
                      <img
                        src={report.sample_article.image}
                        alt=""
                  ...
                  <div className="p-4">
                    <p className="text-[10px] font-semibold tracking-[0.07em] uppercase text-[#86868b] mb-[6px]">
                      Sample article
                    </p>
                    <p className="text-[13px] font-medium text-[#1d1d1f] leading-snug mb-2 line-clamp-2">
                      {report.sample_article.title}
                    </p>
                    <p className="text-[12px] text-[#6e6e73] leading-relaxed line-clamp-3 mb-3">
                      {report.sample_article.summary}
                    </p>
                    <a
                      href={report.sample_article.url}
                      target="_blank"
